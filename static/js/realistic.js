...
camera.position.set(100, 50, 75);// Начальная позиция камеры
...
function createSun() {
    const sunGeometry = new THREE.SphereGeometry(3, 256, 256);
...
function createAsteroid(size, position) {
    const geometry = new THREE.SphereGeometry(size, 16, 16); // Маленький объект
    const material = new THREE.MeshStandardMaterial({ color: 0x888888 }); // Серый материал
    const asteroid = new THREE.Mesh(geometry, material);
    asteroid.position.set(position.x, position.z, position.y);
    scene.add(asteroid);
    return asteroid;
}
...
function createAsteroidBelt() {
    const asteroidCount = 3000; // Увеличиваем количество астероидов для более плотного пояса
    const semiMajorAxisMin = 2.2 * 20; // Минимальное расстояние от Солнца (ближе к Марсу)
    const semiMajorAxisMax = 3.2 * 20; // Максимальное расстояние от Солнца (ближе к Юпитеру)
    
    for (let i = 0; i < asteroidCount; i++) {
        const semiMajorAxis = THREE.MathUtils.lerp(semiMajorAxisMin, semiMajorAxisMax, Math.random()); // Случайная полуось
        const eccentricity = Math.random() * 0.05; // Сниженный эксцентриситет для более круговых орбит
        const inclination = THREE.MathUtils.degToRad(Math.random() * 5); // Наклонение до 5 градусов для более компактного пояса
        const trueAnomaly = THREE.MathUtils.degToRad(Math.random() * 360); // Случайная истинная аномалия
        ...
        const asteroidSize = Math.random() * 0.05 + 0.005; // Более мелкие астероиды

        createAsteroid(asteroidSize, { x, y, z });
    }
}

// Вызов функции создания пояса астероидов
createAsteroidBelt();
...
const saturnRings = createSaturnRings(1.5, 3.5, 0.1, '../static/img/saturn_rings.png');
...
document.querySelector('.realistic-btn').classList.add('active');
